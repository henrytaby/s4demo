...
<form class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed" 
      method="POST" action="classes?action=itemstudentupdatesql"  id="form_modal_interface">
    <input type="hidden" name="item_id" value="${item_id}">

    <div class="modal-body">

        <div class="alert alert-focus" role="alert">
            <c:if test="${type == 'new'}">New</c:if><c:if test="${type == 'update'}">Edit</c:if> Student 
                    ...
                    <c:forEach items="${classesopt}" var='dato'>
                        <c:choose>
                            <c:when test="${dato.id == item.classes.id}">
                                <option value="${dato.id}" selected="">${dato.name} ${dato.last_name}</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${dato.id}">${dato.name} ${dato.last_name}</option>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>                    
                    </select>
...
<script>
    var snippet_form_modal = function() {
        ...
        var options = {
            ...
            , data: {
                accion:'classes?action=itemstudentupdatesql'
                ,id:idficha
                ,type:type
            }
        };
